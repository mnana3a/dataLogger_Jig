...
typedef struct {
    uint8_t fat_num;    // NO. OF FAT TABLES IN THE PARTITION
    uint32_t fat1_start; // FAT1 start position
    uint32_t fat2_start; // FAT2 start position
...
int8_t FAT16_read(uint8_t *ptr, uint16_t bytes);
// Write <bytes> to fat16_buffer, and returns nothing
int8_t FAT16_write(uint16_t bytes);
/* ********************************************************************************************************* */
...
#define FAT16_ERR_FILE_CREATE -5
